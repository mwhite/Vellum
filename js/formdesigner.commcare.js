...
var saveToCaseBlock = function (mug, options) {
    ...
    function getSavedPropertyName(updateDict, prefix) {
        ...
        _(updateDict).each(function (path, name) {
            if (path === mugPath && name.indexOf(prefix) === 0) {
...
var saveToCaseWidget = function (mug, options) {
    ...
    widget.updateValue = function () {
        _updateValue();
        $save.prop('checked', !!widget.getValue());
    };
    ...
    widget.getUIElement = function () {
        ...
        var $saveContainer = $("<div />")
            .addClass('pull-right fd-itextID-checkbox-container')
            .prepend($saveLabel);
...
formdesigner.plugins.commCare = function (options) {
    ...
    that.getSections = function (mug) {
        ...
        //if (mug.__className === "Repeat") {
            //return {
                //slug: "repeat-child-case",
                //type: "accordion",
                //displayName: "Child Cases",
                //properties: ['dataElement/createChildCase'],
                //help: {
                    //title: "Child Cases",
                    //text: "fooo",
                    //link: "https://bar"
        ...
        return [
            ...
            // used for loading case properties in the past
            {
                slug: "load-from-case",
                type: "accordion",
                displayName: "Load from Case (Legacy)",
                properties: ['dataElement/loadFromCase'],
                help: {
                    title: "Load from Case (legacy)",
                    text: "fooo",
                    link: "https://bar"
                }
            }
